...
import MSSqlDB, { QueryDBVariable, SQLInputs }  from '@assets/connections/mssql';
import { HandleError, Sanitize, Captialize, GetPrimaryKey } from '@assets/mixins';
...
export default class MSSqlGenerator {
    ...
    // Constructor
    constructor (context: Context, db: MSSqlDB) {
    // Assign context
        this.ctx = context;
        this.db = db;
        // Determine if working locally
                            ...
                            if (type === 'INSERT') {
                                ...
                                if (i < (keys.length - 1)) {
                                    ...
                                    for (let y = 0; y < ts.PrimaryKey.length; y++) {
                                        ...
                                        const primaryKeyValues: string[] | number[] = GetPrimaryKey(entityId);
    ...
    public GetSchema (entity: string): any {
        ...
        if (this.local) {
            ...
            if (process.env['SchemaDirectory']) {
                ...
                this.errors.push(new Error('Environtment variable - Local environment SchemaDirectory not set'));
            }
            // Try get the file
            ...
            this.errors.push(new Error('Feature not yet available'));
        }
        return result;
    }
